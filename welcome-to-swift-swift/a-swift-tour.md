...
デフォルトで、関数は引数名をそのまま引数ラベルとして使用します。カスタムの引数ラベルを設定したい場合は、引数名の前に引数ラベルを記載してください。引数にラベルが不要な場合は、`_` を書きましょう。
...
関数はネストして使用することができます。ネストした関数は、外側で定義された変数にアクセスすることができます。入れ子関数を使用することで、長くて複雑な関数を整理することができます。
...
単純に値を保持するプロパティ以外に、プロパティが get プロパティ\(_getter_\) と set プロパティ\(_setter_\) を持つこともできます。
...
3. superclass で定義されたプロパティの値を変更。この時点で get/set プロパティのメソッドを使用して他のセットアップ処理を実行
...
> Experiment  
> `ServerResponse` に3つ目の case を追加して、switch 文を変更してください。
