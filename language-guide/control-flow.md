...
特定の条件に基づいてコードの異なるパートを実行するのは役に立つことが多くあります。エラーが起きた時に追加でコードを実行したいこともあるでしょう。値が大きすぎたり、小さすぎたりしたらメッセージを表示したいかもしれません。この場合、コードを条件付き(_conditional_\)することで実現できます。
...
使用する API が実行時に使用可能かどうかに応じて、`if` 文や `guard` 文で使用可能条件(_availability condition_\)を使用して、コードのブロックを条件付きで実行します。コンパイラはこのコードブロックが使用可能かどうかを検証する時に、この使用可能条件からの情報を使います。
