...
特定の条件に基づいてコードの異なるパートを実行するのは役に立つことが多くあります。エラーが起きた時に追加でコードを実行したいこともあるでしょう。値が大き過ぎたり、小さ過ぎたりしたらメッセージを表示したいかもしれません。この場合、コードを条件付き（_conditional_\)することで実現できます。
...
`switch` case は、合致した値を、case の本文で使用するために、一時的な定数または変数に、一致する 1 つまたは複数の値に名前を付けることができます。この挙動は、値を変数や定数にバインドするので、値バインディング（_value binding_\)と呼ばれています。
...
制御転送文（_Control Transfer Statements_\)は、コードの一部の制御を他の部分へ転送させることで、コードの実行順序を変更します。Swift は 5 つの制御転送文があります:
