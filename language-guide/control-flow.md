...
範囲についての詳細は、[Range Operators\(範囲演算子\)](../language-guide/basic-operators#range-operators)を参照ください。
...
`continue`、`break` と `fallthrough` は下記に記載しています。`return` は[Functions\(関数\)](functions.md)、`throw` は[Propagating Errors Using Throwing Functions](../language-guide/error-handling#propagating-errors-using-throwing-functions)に記載しています。
