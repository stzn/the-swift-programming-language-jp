...
上で説明したように、インスタンスメソッドは、特定の型のインスタンスで呼び出すメソッドです。型自体で呼び出されるメソッドを定義することもできます。このようなメソッドは*型メソッド*と呼ばれます。メソッドの `func` キーワードの前に `static` キーワードを記述して、型メソッドを示します。クラスは代わりに `class` キーワードを使用して、サブクラスがそのメソッドのスーパークラスの実装をオーバーライドできるようにすることができます。
