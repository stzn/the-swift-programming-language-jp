...
構造体は、独自のカスタムイニシャライザを定義していない場合、メンバワイズイニシャライザ(*memberwise initializer*)を自動的に受け取ります。デフォルトイニシャライザとは異なり、構造体は、デフォルト値を持たないプロパティが格納されている場合でも、メンバワイズイニシャライザを受け取ります。
...
クラスにはデフォルトでメンバワイズイニシャライザ(*memberwise initializer*)を受け取らないため、`Food` クラスは、`name` という単一の引数を取る指定イニシャライザを提供します。このイニシャライザを使用して、特定の名前で新しい `Food` インスタンスを作成できます。
