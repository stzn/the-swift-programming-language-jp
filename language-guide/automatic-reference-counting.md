...
`capitalCity` にはデフォルトの `nil` 値があるため、新しい `Country` インスタンスは、`Country` インスタンスがイニシャライザ内で `name` プロパティを設定するとすぐに完全に初期化されたと見なされます。これは、`name` プロパティが設定されるとすぐに、`Country` イニシャライザが暗黙の `self` プロパティの参照の受け渡しを開始できることを意味します。そして、`Country` イニシャライザが自身の `capitalCity` プロパティを設定する時に、`self` を `City` イニシャライザの引数の 1 つとして渡すことができます。
