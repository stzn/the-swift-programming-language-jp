...
Closed 範囲演算子は、必要な全ての値を範囲に含め、繰り返し処理を行うときに有用です。例えば `for-in` ループでは:
...
Half-Open 範囲演算子は最後の値のみを書くことでも、片側範囲の形式で書くことができます。両側の値を書いたときと同様に、記載した最後の値は範囲に含まれません。例えば:
...
論理積\(_AND_\)と同様に、論理和\(_OR_\)は式を検討するときに短絡評価を使います。論理和の左側が `true` ならば、既に全体の式も `true` と分かっているので、右側は評価されません。
