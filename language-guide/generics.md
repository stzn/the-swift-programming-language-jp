...
この関数は、[In-Out Parameters](functions.md#in-Out-ParametersIn-Outパラメータ)で説明されているように、in-out パラメータを使用して `a` と `b` の値を交換します。
...
* 山かっこ\(`<>`\)内のジェネリックパラメータのインデックスは、標準ライブラリの `Sequence` プロトコルに準拠した型にする必要があります
* subscript は、その `Indices` 型のインスタンスの単一のパラメータ `indices` を受け取ります
