...
下記の例にある `someBits` と `moreBits` は、異なるビットに `1` を設定されています。ビット論理和演算子はそれらを組み合わせて、符号なし 10 進数の `254` に相当するビット値 `11111110` を作成します。
...
ビット排他的論理和演算子\(`^`\)は、2 つのビット値を比較します。入力されたビット値において、いずれもビットが異なる場合は `1` を、等しい場合は `0` が設定されたビット値を新たに返します:
...
ただし、負数は別の方法で格納されます。それらは `2` と `n` のべき乗から絶対値を引くことによって格納されます。ここで言う `n` は符号ビットを除いたビット値の桁です。8 ビットの数値には 7 つのビット数があるため、これは 2 の 7 乗、つまり `128` です。

`Int8` で整数 `-4` を表す方法は次のとおりです:

![&#x7B26;&#x53F7;&#x4ED8;&#x304D;&#x6574;&#x6570; -4](../assets/bitshiftSignedMinusFour_2x.png)

今回は、符号ビットは `1`\(「負」を意味する\)で、7 つのビット値は `124` の 2 進数値\(`128 - 4`\)です:

![&#x7B26;&#x53F7;&#x4ED8;&#x304D;&#x6574;&#x6570; -4 &#x306E;&#x5024;&#x30D3;&#x30C3;&#x30C8;](../assets/bitshiftSignedMinusFourValue_2x.png)

負数に対するこの変換は、2 の_補数表現_と呼ばれています。負数を表すのに珍しい方法に見えるかもしれませんが、いくつかの利点があります。

まず、\(符号ビットを含む\)8 ビットのすべてを 2 進数加算し、完了したら 8 ビットに収まらないものは全て破棄するだけで、`-1` を `-4` に加算できます:

![&#x7B26;&#x53F7;&#x4ED8;&#x304D;&#x6574;&#x6570;&#x306E;&#x52A0;&#x7B97;](../assets/bitshiftSignedAddition_2x.png)

また、2 の補数表現は負数のビットを正数のように左右にシフトできます。左にシフトするたびにビットを 2 倍し、右にシフトするたびにビットを半分にします。これを実現するために、符号付き整数の右シフトでは追加の規則が適用されます。符号付き整数を右にシフトするとき、符号なし整数と同じ規則を適用しますが、空いた左のビットすべてを 0 ではなく符号ビットで埋めます。
