...
この例の `opaqueJoinedTriangles` の値は、この章の前半の[The Problem That Opaque Types Solve](#the-problem-that-opaque-types-solveopaque-型が解決する問題)セクションのジェネリクスの例の `joinedTriangles` と同じです。ただし、その例の値とは異なり、`flip(_:)` と `join(_:_:)` は、ジェネリックな形状の操作が返す基本的な型を Opaque 型でラップし、それらの型が外部からは見えないようにしています。どちらの関数も、依存する型がジェネリックなのでジェネリック関数で、型引数は `FlippedShape` と `JoinedShape` に必要な型情報を渡します。
