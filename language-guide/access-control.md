...
入れ子型のアクセスレベルは、ラップされている型が public でない限り、ラップされている型と同じです。public 型内の入れ子型には、internal が自動で設定されています。public 型内の入れ子型を public で使用できるようにする場合は、入れ子型を public として明示的に宣言する必要があります。
