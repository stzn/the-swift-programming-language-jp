...
非オプショナルの定数や変数に `nil` を使用することはできません。コード内の定数や変数が特定の条件下で値の欠如を扱う必要がある場合、適切な型のオプショナル値として宣言します。非オプショナル値として宣言された定数や変数は、 `nil` 値を含むことが絶対にできないことが保証されています。非オプショナル値に `nil` を割り当てようとすると、コンパイル時にエラーが発生します。
