...
プロトコルをジェネリック制約として使用することについては、[ジェネリクス\(Generics\)](../language-guide/generics.md)を参照してください。Opaque 型とボックス化されたプロトコル型については、[Opaque 型とボクシングされた型\(Opaque Types and Boxed Types\)](language-guide/opaque-types.md)を参照してください。
