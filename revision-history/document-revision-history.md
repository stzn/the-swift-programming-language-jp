...
* [ジェネリクス\(Generic\)](../language-guide/generics.md)の章の[ジェネリック where 句\(Generic Where Clauses\)](../language-guide/generics.md#generic-where-clauses)セクションと、[ジェネリックパラメータと引数\(Generic Parameters and Arguments\)](../language-reference/generic-parameters-and-arguments.md)の章の説明を更新しました。ジェネリック `where` 句を宣言の最後に書くことができるようになりました
